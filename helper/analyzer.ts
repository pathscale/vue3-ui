...
import { isVueSFC, parseSFC } from "./analyzer-utils";
...
export function getWhitelist(
				...
				if (name === "transition") {
					for (const transition of transitions) {
						for (const suffix of animationSuffixes)
							always.add(transition + suffix);
					}

					if (attrs.name)
						for (const suffix of animationSuffixes)
							always.add(attrs.name + suffix);
	...
	for (const cl of unstable) {
		assert(
			cl in documentedClasses,
			`${cl} not documented in helper/classes.json for ${name}`,
		);
	}
