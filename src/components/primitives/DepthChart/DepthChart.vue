  ...
  setup(props) {
    ...
    const renderGrid = (options) => {
      ...
      for (
        ...
        gridChildren.push(
          lineNode(i, i, 0, height - yAxisGridSpace, gridStyle),
        );
      }
    ...
    const renderAxis = (data, options) => {
      ...
      for (
        let i = originData[0] + xAxisGridSpace;
        i < width;
        i += xAxisGridSpace
      ) {
        axisChildren.push(
          lineNode(i, i, originData[1] - 4, originData[1] + 4, xAxisStyle),
        );
      ...
      for (
        ...
        axisChildren.push(
          lineNode(originData[0] - 4, originData[0] + 4, j, j, yAxisStyle),
        );
        ...
        axisChildren.push(
          textNode(originData[0] + 6, j + 4, number, yAxisStyle),
        );
      }
    ...
    const onMouseMove = ({ offsetX }) => {
      ...
      for (const key of pointMap.keys()) {
        ...
        if (offsetX < x) {
          ...
          // draw the tooltip
          if (tipType === "mouse") {
            const tipContextFill =
              side === "buy" ? tipBuyBgColor : tipSellBgColor;
            const tipContextStroke =
            ...
            const tipsPriceWidth =
              tipsPriceText.length * tipContextFontSize + 20;
            const tipsAmountWidth =
              tipsAmountText.length * tipContextFontSize + 15;
            const lineHeight = 20;
            ...
            tipChildren.push(
              textNode(x, height + 19, tipsPriceText, toopTipTextStyle),
            );
            ...
            tipChildren.push(
              textNode(30, y + 4, tipsAmountText, toopTipTextStyle),
            );
          }
